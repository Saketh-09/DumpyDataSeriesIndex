...
class Node {
    Node(){;}
    explicit Node(int _layer) {
        layer = _layer;
    }
    Node(int _layer, int pid) {
        layer = _layer;
        partition_id = pid;
        bits_cardinality[0] = -1;
    }
    // only for node in 1st layer
    Node(int _layer, int _size, int _id) {
        layer = _layer;
        size = _size;
        id = _id;
    ...
    Node(const Node* parent, int _size, int _id){
    ...
    Node(const Node* parent, int pid){
        layer = parent->layer + 1;
        partition_id = pid;
    ...
    PAA_INFO* statPaa();
    void chooseSegment(PAA_INFO *paa, int chosen_number);
    static int chooseOneSegment(PAA_INFO *node);
    void generateSaxAndCardIn1stLayer(int new_id);
    void generateSaxAndCardinality(Node *node, int new_id);
    void generateSaxAndCardIn1stLayer4LeafNode(int new_id);
    void generateSaxAndCardinality4LeafNode(Node *node, int new_id);
    ...
    void growIndex();
    void growIndexFuzzy(unordered_map<Node *, NODE_RECORDER> &navigating_table, vector<vector<int>> *g);
    ...
    void fuzzy(partUnit *part_units, vector<int> &actual_sizes, vector<vector<int>> &node_offsets,
               ...
               unordered_map<Node *, NODE_RECORDER> &navigating_tbl) const;
...
public:
    ...
    Node *route(const unsigned short *_sax);
    static Node *BuildingIndex(string &datafn, string &saxfn);

    [[nodiscard]] string getFileName() const{
        if(layer == 1)  return "1_" + to_string(partition_id);
        return to_string(layer) + "-" + file_id;
    }
    [[nodiscard]] bool isLeafNode() const   {return ch.empty();}
    ...
    static Node*  BuildIndexFuzzy(const string & datafn, const string & saxfn, const string &paafn, vector<vector<int>>* g);
    ...
    static Node *loadFromDisk(const string &saxfn, const string &idxfn, bool need_sax);
    ...
    Node *route1step(const unsigned short *_sax);
    ...
    void determineFanout(int *lambda_minimum, int *lambda_max) const;
...
struct NODE_RECORDER{
    ...
    explicit NODE_RECORDER(int act_size, Node *node) {
