...
double MathUtil::deviation(double* timeSeries, int start, int end) {
    double mean = avg(timeSeries, start, end);
...
int MathUtil::split(const std::vector<int>& points, int minLength, int len, std::vector<int>& ret) {
    std::vector<int> newPoints(len + len);
    int c = 0;
    for (int i = 0; i < len; i++) {
        ...
        if (length >= minLength * 2) {
            ...
            if (i > 0)
                start = points[i - 1];
            newPoints[c++] = start + length / 2;
        }
        newPoints[c++] = points[i];
    }
    ret.resize(c);
    std::copy(newPoints.begin(), newPoints.begin() + c, ret.begin());
    return c;
}
...
double MathUtil::errorRatio(const std::vector<PqItemSeries*>& approx, const std::vector<PqItemSeries*>& exact, int k) {
    double sum = 0;
...
double MathUtil::tightness(const std::vector<PqItemSeries*>& approx, const std::vector<PqItemSeries*>& exact, int k) {
    double sum = 0;
    for (int i = 0; i < approx.size(); ++i) {
        if (approx[i]->dist == 0 && exact[i]->dist == 0)
            sum += 1;
        else
            sum += (sqrt(exact[i]->dist) / sqrt(approx[i]->dist));
    }
    return sum / k;
}
