...
double* ConversionUtil::readFromstring(string str) {
    vector<string> strings = split(str, ",");
    auto* ret = new double[strings.size()];
...
double * ConversionUtil::readDoubleFromFileAtOnce() {
...
void ConversionUtil::id2Sax2(int id, unsigned short *sax, int n){
    int i=0;
    while(i<n) {
        int temp = id >> 1;
        sax[n - i - 1] = (sax[n - i - 1] << 1) + ((temp + temp == id) ? 0 : 1);
        id = temp;
        i++;
    }
}

float * ConversionUtil::paaFromTsFloat(const float* ts, int tsLengthPerSegment, int n){
    // Create PAA representation
    auto* paa = new float [n];
    int s=0, i;
    while(s<n) {
        paa[s] = 0;
        i=0;
        while (i<tsLengthPerSegment) {
...
double * ConversionUtil::paaFromTs(const float* ts, int tsLengthPerSegment, int segmentNum) {
    // Create PAA representation
...
void ConversionUtil::paaFromTs(const float* ts, float *paa, int tsLengthPerSegment, int segmentNum){
    // Create PAA representation
...
void ConversionUtil::paaAndSaxFromTs(const float* ts, float *paa, unsigned short *sax, int tsLengthPerSegment, int segmentNum, int cardinality){
    // Create PAA representation
...
vector<int> * ConversionUtil::saxFromTs(float*ts, int tsLengthPerSegment, int segmentNum, int cardinality)
{
    // Create PAA representation
...
void ConversionUtil::saxFromTs(const float*ts, unsigned short *sax, int tsLengthPerSegment, int segmentNum, int cardinality)
{
    // Create PAA representation
...
vector<unsigned short> * ConversionUtil::saxFromPaa(float *paa, int segmentNum, int cardinality)
{
...
int ConversionUtil::findFirstGE(const double* array, int start, int length, double target) // satisfy condition: array[?] >= target  and the first one
{
...
int ConversionUtil::findFirstGE(const int* array, int start, int length, int target) // satisfy condition: array[?] >= target  and the first one
{
    int end = start + length - 1;
    while (start <= end) {
        int mid = (start + end) / 2;
...
int ConversionUtil::invSaxHeadFromSax(vector<int> *sax, int bitsCardinality, int segmentNum)
{
...
int ConversionUtil::invSaxHeadFromSax(const unsigned short *sax, int bitsCardinality, int segmentNum)
{
    int i=bitsCardinality-1,s = 0,n;
    for (int j=0; j < segmentNum; j++)
    {
...
int ConversionUtil::invSaxHeadkFromSax(const unsigned short *sax, int bitsCardinality, int segmentNum, int k)
{
...
int ConversionUtil::invSaxHeadFromPaa(const float *paa, int tsLengthPerSegment, int segmentNum) {
    int res = 0;
...
double ConversionUtil::LowerBound_Paa_iSax(const float *paa, const unsigned short *sax){
...
double ConversionUtil::LowerBound_Paa_iSax(const float *paa, const unsigned short *sax, const int* bits_cardinality, vector<int>&chosen_segs, int new_id){
    double frontCoef = (double)Const::tsLength / Const::segmentNum; // n / w
    double paaValue, lb, ub;
    double sum = 0;
...
double ConversionUtil::LowerBound_Paa_iSax(const float *paa, const unsigned short *sax, int bits_cardinality) {
    double frontCoef = (double)Const::tsLength / Const::segmentNum; // n / w
...
double ConversionUtil::LowerBound_Paa_iSax(const float *paa, const unsigned short *sax, const int* bits_cardinality) {
    double frontCoef = (double)Const::tsLength / Const::segmentNum; // n / w
    double paaValue, lb, ub;
    double sum = 0;
...
double ConversionUtil::minidist_paa_to_isax_DTW(const double *paaU, const double *paaL ,  const unsigned short *sax,
...
double ConversionUtil::getMinDist1stLayerDTW(const double *paaU, const double *paaL, int id){
    double breakpoint_lower, breakpoint_upper;
    double distance = 0;
    for(int i=Const::segmentNum-1;i>=0;--i){
        if(id %2 == 1){
...
double ConversionUtil::getMinDist1stLayer(const float *paa, int id){
    double ret = 0;
    double frontCoef = (double)Const::tsLength / Const::segmentNum; // n / w
    for(int i=Const::segmentNum-1;i>=0;--i){
        if(id %2 == 1){
...
void ConversionUtil::getValueRange(int sax_single, int bits_cardinality, double *lb, double *ub){
...
// return the new id
int ConversionUtil::extendSax(float *paa, const int *bits_cardinality, vector<int> &segments) {
...
// return the new id
int ConversionUtil::extendSax(const unsigned short *sax, const int *bits_cardinality, vector<int> &segments) {
    int res = 0, sw;
    for(int segment:segments){
...
// return the new id
int ConversionUtil::extendSax(const unsigned short *sax, const int *bits_cardinality) {
    int res = 0, sw;
...
double ConversionUtil::getMidLineFromSaxSymbolbc8(unsigned short symbol){
    double numer;
    assert(symbol >=0 && symbol <=255);
    if(symbol == 0) {
        numer = ConversionUtil::bp8[0] - (ConversionUtil::bp8[1] - ConversionUtil::bp8[0]);
    }
    else if(symbol == 255) {
        numer = ConversionUtil::bp8[254] + (ConversionUtil::bp8[254] - ConversionUtil::bp8[253]);
    }
    else {
        numer = (ConversionUtil::bp8[symbol - 1] + ConversionUtil::bp8[symbol]) / 2.0;
    }
    return numer;
}

int ConversionUtil::getNewId(const float *paa, const float *split_line){
    int ret = 0;
    for(int i=0;i<Const::segmentNum;++i){
        ret <<= 1;
...
int ConversionUtil::getNewId(const float *paa, const float *split_line, vector<int>&segments){
    int ret = 0;
    for(int i:segments){
        ret <<= 1;
        if(paa[i] > split_line[i])  ret += 1;
...
void ConversionUtil::saxPrint(int* sax, int bits_cardinality, int segment_num)
{
    int i;
...
void ConversionUtil::printBinary(long n, int size) {
    char b[size + 1];
    b[size] = 0;
    int i;
    for (i=0; i<size; i++) {
...
void ConversionUtil::generateSaxFile(const string &fn , const string &output){
...
void ConversionUtil::generatePaaFile(const string &fn, const string &output) {
    long fs = FileUtil::getFileSize(fn.c_str());
    int num = fs / Const::tsLengthBytes;
    cout << "Total number is " << num << endl;
    float ts[Const::tsLength];
...
void ConversionUtil::lower_upper_lemire(const float *t, int len, int r, float *l, float *u)
{
    dequeue du, dl;
