...
int TimeSeriesUtil::intersectionTsSetsCardinality(const vector<PqItemSeries *> &tsSet1,
                                                  const vector<PqItemSeries *> &tsSet2) {
...
float TimeSeriesUtil::lb_keogh_data_bound(float *qo, float *upperLemire, float *lowerLemire, float *cb, int len,
    ...
    for (; i < len && lb < bsf; i++) {
        float uu = upperLemire[i];
        float ll = lowerLemire[i];
        float d = 0;
        if (qo[i] > uu) {
            d = dist(qo[i], uu);
        } else if (qo[i] < ll) {
...
double TimeSeriesUtil::dtw(const float *A, const float *B, int len, int r, double bsf) {
    ...
    double final_dtw = cost_prev[k - 1];
    free(cost);
    free(cost_prev);
    return final_dtw;
}
